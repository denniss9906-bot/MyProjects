...
function onJsonMovie(json){
   ...
   //Elaboro i risultati e li visualizzo a schermo
    ...
    if(json.result[elem].posterURLs['500'] != undefined && json.result[elem].title != undefined){
    const movie=document.createElement("div");
    movie.classList.add("movie");

    movie.dataset.id=json.result[elem].tmdbId;
...
function search(event){
    ...
    fetch(SEARCH + "/" + encodeURIComponent(form_data.get('search'))).then(onResponse).then(onJsonMovie);
...
function addWatchlist(event){
    ...
    movieForm.append('_token',CSRF_TOKEN);
    fetch( ADD,{method:'post',body:movieForm}).then(onDBResponse).then(onDBjson);
    currenttargetevent=event.currentTarget.firstChild;
...
function onDBjson(json){
    ...
    if(!json.exists){
        currenttargetevent.classList.remove("add_icon");
        currenttargetevent.classList.add("add_icon_exists");
       return console.log("Elemento già presente nel database");
    }else{
      console.log("Elemento inserito correttamente");
      currenttargetevent.classList.remove("add_icon");
      currenttargetevent.classList.add("add_icon_saved");
