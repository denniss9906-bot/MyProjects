...
function verifyUser(){
    ...
    if(usernamePattern.test(userInput.value)){
     fetch(VERIFY_USER + "/" + encodeURIComponent(userInput.value)).then(onResponse).then(onJsonUser);
...
function verifyEmail(){
    ...
    if(mailPattern.test(mailInput.value)){
      fetch(VERIFY_MAIL + "/" + encodeURIComponent(mailInput.value)).then(onResponse).then(onJsonMail);
    }else{
        mailInput.style.border="solid 2px red";
...
function verifyConfPassword(){
    ...
    if(confInput.value === document.querySelector("#password input").value && confInput.value.length > 0){
...
document.querySelector('form').addEventListener('submit',controlladati);

function controlladati(event){
    if (!formStatus['upload']) {
